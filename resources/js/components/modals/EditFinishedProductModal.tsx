...
export function EditFinishedProductModal({ open, onClose, product, onSuccess }: Props) {
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [primaryImage, setPrimaryImage] = useState<File | null>(null);
  const [primaryImagePreview, setPrimaryImagePreview] = useState<string | null>(product.primary_image || null);
  const [primaryImageDeleted, setPrimaryImageDeleted] = useState(false);
  ...
  useEffect(() => {
    if (open) {
      ...
      setPrimaryImagePreview(product.primary_image || null);
      setPrimaryImageDeleted(false);
  ...
  const removeAdditionalImage = (index: number) => {
    ...
    // Check if it's an existing image (URL string from server) or a new file (data URL)
    const isExistingImage = imageUrl && imageUrl.startsWith('http');
    ...
    if (isExistingImage) {
      ...
      // Remove from new files list (find matching index in new files)
      const newFileIndex = additionalImagePreviews
        .slice(0, index)
        .filter((url) => !url.startsWith('http')).length;
      setAdditionalImages((prev) => prev.filter((_, i) => i !== newFileIndex));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (primaryImageDeleted) {
        data.append('delete_primary_image', '1');
      }
      ...
      // Send remaining existing images (ones that weren't deleted)
      existingImages.forEach((imageUrl, index) => {
        data.append(`existing_images[${index}]`, imageUrl);
      });
      ...
      await axios.post(`/admin/finished-products/${product.id}`, data, {
        headers: {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update finished product details for {product.reference}
          </DialogDescription>
        </DialogHeader>
